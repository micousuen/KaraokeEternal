...
describe('rollingCues', () => {
  it('advances two rows independently', () => {
    expect(rollingCues(words([
      ...
      { start: 0, end: 1, text: 'One.\nTwo.', activeRow: 0, activeStart: 0, activeEnd: 4 },
      { start: 1, end: 2, text: 'Three.\nTwo.', activeRow: 1, activeStart: 0, activeEnd: 4 },
      { start: 2, end: 3, text: 'Three.', activeRow: 0, activeStart: 0, activeEnd: 6 },
    ])
  })
  ...
  it('advances the active character range at every word timestamp', () => {
    expect(rollingCues(words([
      ...
      { start: 0, end: 0.5, text: 'Sing with me', activeRow: 0, activeStart: 0, activeEnd: 4 },
      { start: 0.5, end: 1, text: 'Sing with me', activeRow: 0, activeStart: 5, activeEnd: 9 },
      { start: 1, end: 1.4, text: 'Sing with me', activeRow: 0, activeStart: 10, activeEnd: 12 },
    ])
  })
...
it('writes the custom active-row marker into valid SRT timing lines', () => {
  expect(createRollingSrt(words([['Hello.', 1.2345, 2.5]]), 'en', 36, 12)).toBe(
    '1\n00:00:01,235 --> 00:00:02,500 A0 C0-6\nHello.\n',
  )
})
